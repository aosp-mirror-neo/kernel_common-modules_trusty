...
/*
 * Copied from drivers/dma-buf/heaps/system_heap.c with minimal
 * changes (only a few extra pKVM protection hypercalls) for example
 * purposes.
 */
...
static void system_heap_dma_buf_release(struct dma_buf *dmabuf)
{
	...
	for_each_sgtable_sg(table, sg, i) {
		...
		pkvm_el2_mod_call(unprotect_page_hvc, page_to_pfn(page), 1 << compound_order(page));
					       ...
					       bool uncached)
{
	...
	list_for_each_entry_safe(page, tmp_page, &pages, lru) {
		...
		pkvm_el2_mod_call(protect_page_hvc, page_to_pfn(page), 1 << compound_order(page));
	}
...
const struct dma_heap_ops system_heap_modified_ops = {
